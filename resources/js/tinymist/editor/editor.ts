...
export class TinymistEditorUI {
    ...
    async setupCodeMirror() {
        try {
            ...
            window.$tmEventBus.emit("tinymist-console-log", { type: "info", message: "CodeMirror editor initialized" });
    ...
    setupListeners() {
        window.$tmEventBus.listen("tinymist-sync-full-state", this.syncFullStateFromServer);
        window.$tmEventBus.listen("tinymist-prune-snapshots", this.pruneSnapshots);
        window.$tmEventBus.listen("tinymist-fallback-enable", (enabled: boolean) => this.fallbackEnabled = enabled);
        window.$tmEventBus.listen("tinymist-active-file-change", this.setActiveFile);
        window.$tmEventBus.listen("editor::insert", this.insertFromEditorEvent);
        window.$tmEventBus.listen("tinymist-attachment-reset-file", this.resetAttachmentFileFromServer);
    ...
    removeListeners() {
        window.$tmEventBus.remove("tinymist-sync-full-state", this.syncFullStateFromServer);
        window.$tmEventBus.remove("tinymist-prune-snapshots", this.pruneSnapshots);
        window.$tmEventBus.remove("tinymist-active-file-change", this.setActiveFile);
        window.$tmEventBus.remove("editor::insert", this.insertFromEditorEvent);
        window.$tmEventBus.remove("tinymist-attachment-reset-file", this.resetAttachmentFileFromServer);
    ...
    onInput() {
        ...
        if (this.activeFileName === this.entryFileName) {
            window.$tmEventBus.emit("editor-tinymist-change", "");
        }
    }
    ...
    buttonsListener(event: Event) {
        ...
        switch (action) {
            ...
            case "changeCodeMirrorSettings":
                window.$tmEventBus.emit("tinymist-theme-settings-open");
                break;
            default:
    ...
    public setActiveFile(payload: { fileName: string; url: string }): void {
        ...
        window.$tmEventBus.emit("tinymist-sync-open-file", { fileName: fileName });
    }
    ...
    private flushPendingChanges(fileName: string): void {
        ...
        if (this.fallbackEnabled && fileName === this.entryFileName) {
            window.$tmEventBus.emit("tinymist-fallback-compile", {
                content: state.currentContent,
                docVersion: state.docVersion,
            });
        } else if (!this.fallbackEnabled) {
            window.$tmEventBus.emit("tinymist-text-diff", {
                fileName,
    ...
    private emitDirtyState(fileName: string, isDirty: boolean): void {
        window.$tmEventBus.emit("tinymist-attachment-dirty-state", {
            fileName,
            isDirty,
        });
    }
    ...
    private resetAttachmentFileFromServer(payload: { fileName?: string }): void {
        ...
        window.$tmEventBus.emit("tinymist-sync-open-file", { fileName });
    }
    ...
    onCursorPositionChange(state: any) {
        ...
        window.$tmEventBus.emit("tinymist-control", {
