...
1. **Laravel bootstraps the file**
  ...
  - File path: storage/app/tinymist/page_<id>/entry.typ
...
4. **Code Mirror in the back end file-manager.ts**  checks document version for the sync, and writes updates to the of the file in the `storage/app/tinymist/page_<id>/entry.typ`. If the version of the file is bigger on the back end, it sends "full sync" document to the browser to sync it.

5. `storage/app/tinymist/page_#/entry.typ` is a real file, not a virtual one. That allows for tinymist preview to watch it and send incremental updates. As soon as LSP server is notified of changes it will provide diagnostics. It can also provide other things if queried for them specifically.
...
3.**Server apply** – In the Node handler, load the current text into `@codemirror/state`’s `Text` object, reconstruct the `ChangeSet` from the JSON (`ChangeSet.fromJSON`), apply it, update the stored version, and write the resulting string back to `storage/app/tinymist/page_<id>/entry.typ` using `fs.promises.writeFile`. Reject messages whose `docVersion` is behind the server’s version and request a resync.
...
- Node writes the authoritative file under storage/app/tinymist/page_<id>/entry.typ.
