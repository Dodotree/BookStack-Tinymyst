...
export class TinymistPreviewRenderer {
    ...
    async initialize() {
        try {
            ...
            console.log("[Preview] typst-ts-renderer initialized");

            await this.connectDataPlane();

        } catch (error) {
            console.error("[Preview] Failed to initialize typst-ts-renderer:", error);
            throw error;
        }
    }
        ...
        return new Promise((resolve, reject) => {
            try {
                ...
                this.dataWs.onopen = () => {
                    ...
                    console.log(`[Preview] Connected to Tinymist data plane at ${wsUrl}`);
                ...
                this.dataWs.onmessage = (event) => {
                    ...
                    this.processingQueue = this.processingQueue
                        .then(async () => {
                            console.log("[Preview] Data plane message received", event);

                            if (data instanceof ArrayBuffer) {
                                await this.handleBinaryMessage(new Uint8Array(data));
                            } else if (data instanceof Blob) {
                                console.log("[Preview] Data plane message blob data:", data);
                                const buffer = await data.arrayBuffer();
                                await this.handleBinaryMessage(new Uint8Array(buffer));
                            } else if (typeof data === "string") {
                                console.log("[Preview] Data plane message string data:", data);
                            }
                        })
                        .catch((err) => {
                            console.error("[Preview] Failed to process message:", err);
                        });
                };

                this.dataWs.onerror = (error) => {
                    console.error("[Preview] Data plane error:", error);
                    reject(error);
                };
                ...
                this.dataWs.onclose = (event) => {
                    ...
                    console.warn(
                        `[Preview] Data plane closed: code=${event.code}(${errCodes[event.code] || "Unknown"}), reason=${event.reason}`
                    );
                    if (event.code !== 1000) {
    ...
    private async handleBinaryMessage(msg: Uint8Array) {
        try {
            ...
            if (commaIndex === -1) {
                console.warn("[Preview] Invalid data plane message format", msg);
                return;
            }
            ...
            switch (command) {
                case 'diff-v1':
                    console.log(`[Preview] Received diff-v1 (${payload.length} bytes)`);
                    break;

                case 'new':
                    console.log(`[Preview] Received new document (${payload.length} bytes)`);
                    break;

                case 'partial-rendering':
                    const enabled = new TextDecoder().decode(payload) === 'true';
                    console.log(`[Preview] Partial rendering: ${enabled}`);
                    break;

                case 'jump':
                    const coords = new TextDecoder().decode(payload).split(" ");
                    const [page, x, y] = coords.map(Number);
                    console.log(`[Preview] Jump to page ${page}, x: ${x}, y: ${y}`);
                    break;
            }
            ...
            if (command === 'diff-v1' || command === 'new') {
                ...
                if (!this.renderer) {
                    console.warn("[Preview] Renderer not ready");
                    return;
                }

                console.log(`[Preview] Processing ${command} (${payload.length} bytes)`);
                ...
                try {
                    ...
                    if (isDiff && !this.hasInitialDocument) {
                        console.warn('[Preview] Treating first diff as full reset');
                        action = 'reset';
                    }

                    console.log(`[Preview] Applying ${action} with ${payload.length} bytes...`);
                    this.renderer!.manipulateData({
                        renderSession: session,
                        action,
                        data: payload,
                    });
                    console.log('[Preview] Data applied successfully');
                    ...
                    try {
                        ...
                        console.log('[Preview] Custom data:', customData);
                    } catch (e) {
                        console.log('[Preview] No custom data:', e);
                    }

                    console.log('[Preview] Rendering to SVG...');
                    const svg = await session.renderSvg({});
                    console.log('[Preview] SVG length:', svg.length);
            ...
            console.error("[Preview] Failed to handle binary message:", error);
        }
    }
    ...
    private handleReconnect() {
        if (this.reconnectAttempts < this.maxReconnectAttempts) {
            ...
            console.log(
                `[Preview] Reconnecting... Attempt ${this.reconnectAttempts}`
            );

            this.connectionTimeout = setTimeout(() => {
                this.connectDataPlane().catch((err) => {
                    console.error("[Preview] Reconnection failed:", err);
                });
            }, 1000 * this.reconnectAttempts);
        } else {
            console.error(
                "[Preview] Max reconnection attempts reached"
            );
        }
    }
    ...
    private async ensureSession(): Promise<RenderSession> {
        ...
        if (!this.sessionPromise) {
            console.log('[Preview] Creating persistent session');
